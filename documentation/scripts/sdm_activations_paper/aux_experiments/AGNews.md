...
We additionally examine the standard 4-class ($|\mathcal{Y}| = 4$) AGNews dataset (available at https://huggingface.co/datasets/fancyzhx/ag_news), which was suggested by someone as a possible additional experiment to consider. This experiment demonstrates the behavior of the SDM estimator on a multi-class classification dataset with $|\mathcal{Y}| > 2$.
