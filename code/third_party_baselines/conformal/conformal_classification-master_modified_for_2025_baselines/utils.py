...
def validate(val_loader, model, print_bool):
    ...
    with torch.no_grad():
        ...
        for i, (x, target) in enumerate(val_loader):
            ...
            prec1, prec5 = accuracy(output, target, topk=(1, 2))  #topk=(1, 5))
            ...
            if print_bool:
                ...
                # print(f'\rN: {N} | Time: {batch_time.val:.3f} ({batch_time.avg:.3f}) | Cvg@1: {top1.val:.3f} ({top1.avg:.3f}) | Cvg@2: {top5.val:.3f} ({top5.avg:.3f}) | Cvg@RAPS: {coverage.val:.3f} ({coverage.avg:.3f}) | Size@RAPS: {size.val:.3f} ({size.avg:.3f})', end='')
                print(f'N: {N} | Time: {batch_time.val:.3f} ({batch_time.avg:.3f}) | Cvg@1: {top1.val:.3f} '
                      f'({top1.avg:.3f}) | Cvg@2: {top5.val:.3f} ({top5.avg:.3f}) | Cvg@RAPS: {coverage.val:.3f} '
                      f'({coverage.avg:.3f}) | Size@RAPS: {size.val:.3f} ({size.avg:.3f})')
...
def get_logits_targets(model, loader):
    cuda_is_available = torch.cuda.is_available()
    print(f"Pre-computing logits with {model.class_size}-class classification.")
    # logits = torch.zeros((len(loader.dataset), 1000)) # 1000 classes in Imagenet.
    # logits = torch.zeros((len(loader.dataset), 3))  # number of classes
    logits = torch.zeros((len(loader.dataset), model.class_size))
