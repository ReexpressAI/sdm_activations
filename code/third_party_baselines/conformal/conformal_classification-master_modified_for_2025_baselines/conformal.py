...
class ConformalModel(nn.Module):
    def __init__(self, model, calib_loader, alpha, kreg=None, lamda=None, randomized=True, allow_zero_sets=False, pct_paramtune = 0.3, batch_size=32, lamda_criterion='size'):
        ...
        # the extra .dataset may be a typo, unless ImageNet class is different: self.num_classes = len(calib_loader.dataset.dataset.classes)
        self.num_classes = model.class_size
        ...
        if kreg == None or lamda == None:
            ...
            print(f"lambda value determined by grid search: {lamda}")
    ...
    if torch.cuda.is_available():
        ...
        for iter in range(max_iters):
            ...
            if abs(T_old - T.item()) < epsilon:
                break
    else:  # just a copy of the above to get rid of cuda()
...
def get_violation(cmodel, loader_paramtune, strata, alpha):
    ...
    for logit, target in loader_paramtune:
        ...
        df = pd.concat([df, pd.DataFrame(batch_df)], ignore_index=True)  # August 2024
    wc_violation = 0
