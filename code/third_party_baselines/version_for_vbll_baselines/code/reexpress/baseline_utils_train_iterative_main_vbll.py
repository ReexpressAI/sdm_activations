    ...
    if not options.eval_only:
        ...
        for shuffle_index in range(max(options.number_of_random_shuffles, 1)):
            if options.continue_training:
                model = baseline_utils_model_vbll.load_baseline_model_torch(options.model_dir, torch.device("cpu"))
            ...
            min_held_out_balanced_cross_entropy_loss, min_held_out_balanced_cross_entropy_loss_epoch = \
                baseline_utils_train_main_vbll.train(options, train_embeddings=train_embeddings,
            ...
            # Note that for simplicity we are overloading the variable naming here.
            print(f"Min calibration VBLL loss (used to determine shuffle index: "
                  f"True) of "
            ...
            if save_this_shuffle_index:
                ...
                model = baseline_utils_model_vbll.load_baseline_model_torch(shuffle_index_model_dir, torch.device("cpu"), is_discriminative_vbll_model=options.is_discriminative_vbll_model)
                baseline_utils_model_vbll.save_baseline_model(model, options.model_dir)
                ...
                if not options.do_not_shuffle_data and not options.do_not_resave_shuffled_data:
                    baseline_utils_model_vbll.save_json_lines(os.path.join(best_iteration_data_dir, "train.jsonl"),
                                                              train_data_json_list)
                    baseline_utils_model_vbll.save_json_lines(os.path.join(best_iteration_data_dir,
                                                                           "calibration.jsonl"),
