...
def train_iterative_main(options, rng, main_device=None, train_uuids=None, calibration_uuids=None,
                         train_data_json_list=None, calibration_data_json_list=None,
                         train_embeddings=None, calibration_embeddings=None,
                         train_labels=None, calibration_labels=None, training_embedding_summary_stats=None):
    ...
    if not options.eval_only:
        ...
        for shuffle_index in range(max(options.number_of_random_shuffles, 1)):
            ...
            if options.is_sdm_network_verification_layer:
                ...
                print(f"(Training SDM verification layer so D_tr and D_ca are NOT reshuffled every iteration here. "
                      f"Those splits are, however, reshuffled every epoch in the outer LLM training loop.)")
                train_meta_data = {"uuids": train_uuids}
                calibration_meta_data = {"uuids": calibration_uuids}
            else:
            ...
            for class_label in range(options.class_size):
                print(
                    f"Training class {class_label}: {torch.sum(train_labels == class_label).item()} documents")
                print(
                    f"Calibration class {class_label}: {torch.sum(calibration_labels == class_label).item()} documents")
