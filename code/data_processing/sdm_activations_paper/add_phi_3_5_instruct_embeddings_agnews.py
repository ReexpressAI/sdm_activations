...
def get_agreement_model_embedding(model, tokenizer, document_text: str, device):
    ...
    # In this case we need to generate two tokens, since the first is the underscore.
    ...
    class0_id = tokenizer.vocab['0']
    class1_id = tokenizer.vocab['1']
    class2_id = tokenizer.vocab['2']
    class3_id = tokenizer.vocab['3']
    # note the 1
    probs = torch.softmax(scores[1], dim=-1)
    # For reference, this reproduces the final output as input to the softmax:
    # model.lm_head(hidden_states[1][-1][0][-1, :])  # [generation_step][layer][batch, seq, hidden]
    ...
    embedding = torch.cat([
        torch.mean(hidden_states[1][-1][0], dim=0).unsqueeze(0),
        hidden_states[1][-1][0][-1, :].unsqueeze(0)
    ], dim=-1).to(torch.float32)
    embedding = [float(x) for x in embedding[0].cpu().numpy().tolist()]
    assert len(embedding) == EXPECTED_EMBEDDING_SIZE
    # The attributes are as follows:
    # no_prob :: yes_prob
    attributes = torch.cat([probs[0:1, class0_id].unsqueeze(0),
                            probs[0:1, class1_id].unsqueeze(0),
                            probs[0:1, class2_id].unsqueeze(0),
                            probs[0:1, class3_id].unsqueeze(0)], dim=-1)
    llm_classification = torch.argmax(attributes)
    attributes = [float(x) for x in attributes[0].cpu().numpy().tolist()]
    assert len(attributes) == 4
    # note the 1
    score_index = 1
    logits = torch.cat([scores[score_index][0:1, class0_id].unsqueeze(0),
                        scores[score_index][0:1, class1_id].unsqueeze(0),
                        scores[score_index][0:1, class2_id].unsqueeze(0),
                        scores[score_index][0:1, class3_id].unsqueeze(0)], dim=-1)
    ...
    assert len(logits) == 4
    # llm_classification = torch.argmax(attributes)  # probs[0:1, no_id] < probs[0:1, yes_id]
...
def get_prompt(document, dataset):
    ...
    elif dataset == AGNEWS_DATA:
        return f"Here is a news article. <article> {document} </article> Classify the article into one of the following four categories: World (0), Sports (1), Business (2), Sci/Tech (3). Only respond with the category number."
    else:
        assert False
...
def construct_embedding_streaming(options, model, tokenizer):
    ...
    with codecs.open(options.input_file, "r", encoding="utf-8") as f:
        for line in f:
            ...
            # try:
            if True:
                prompt = get_prompt(json_obj[REEXPRESS_DOCUMENT_KEY].strip(), options.dataset)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--class_size", default=4, type=int, help="class_size")
    ...
    assert options.dataset == AGNEWS_DATA
