...
def get_agreement_model_embedding(model, tokenizer, document_text: str, device):
    ...
    # Here, the starting no has a prefix underscore. Other models may differ. tokenizer.convert_ids_to_tokens([1770])
    no_id = tokenizer.vocab['▁No']
    yes_id = tokenizer.vocab['▁Yes']  # tokenizer.convert_ids_to_tokens([5592])
    probs = torch.softmax(scores[0], dim=-1)
    # For reference, this reproduces the final output as input to the softmax:
    # model.lm_head(hidden_states[0][-1][0][-1, :]).unsqueeze(0).to(torch.float32)
...
def construct_embedding_streaming(options, model, tokenizer):
    ...
    with codecs.open(options.input_file, "r", encoding="utf-8") as f:
        for line in f:
            ...
            else: #except:  # In principle, this case should never occur with this model, so we remove the try/catch.
    ...
    model_label = "Mixtral-8x7B-Instruct-v0.1"
...
def get_model():
    ...
    model = AutoModelForCausalLM.from_pretrained(
        ...
        max_memory={0: "40GB", 1: "79GB"}  # Conservative limit on cuda:0
    )
